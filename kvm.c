...
kvm_ioctl_decode_irq_line_status(struct tcb *const tcp, const unsigned int code, const kernel_ulong_t arg)
{
	...
	if (entering(tcp))
		irq_level = xmalloc(sizeof(*irq_level));
	else
	...
	if (entering_irq_level) {
		if (syserror(tcp) || code == KVM_IRQ_LINE)
			tprintf(", {{irq=%u}, level=%u}",
				entering_irq_level->irq,
				irq_level->level);
		else if (code == KVM_IRQ_LINE_STATUS)
...
kvm_ioctl_decode_mp_state(struct tcb *const tcp, const unsigned int code, const kernel_ulong_t arg)
{
	...
	if (umove(tcp, arg, &mp_state) < 0)
		return RVAL_DECODED;

	PRINT_FIELD_XVAL(", {", mp_state, mp_state, kvm_mp_states,
			 "KVM_MP_STATE_???");
	tprints("}");
	return RVAL_IOCTL_DECODED;
}
...
kvm_run_structure_decode_io(struct tcb *const tcp, struct kvm_run *run_struct)
{
	...
	PRINT_FIELD_X(", ",  run_struct->io, data_offset);
	tprints("\n");
}
...
kvm_run_structure_decode_main(struct tcb * tcp, struct kvm_run * vcpu_run_struct)
{
	...
	case KVM_EXIT_FAIL_ENTRY:
		kvm_run_structure_decode_fail_entry(tcp, vcpu_run_struct);
		break;
	case KVM_EXIT_EXCEPTION:
		kvm_run_structure_decode_exception(tcp, vcpu_run_struct);
		break;
	case KVM_EXIT_IO:
		kvm_run_structure_decode_io(tcp, vcpu_run_struct);
		break;
	case KVM_EXIT_MMIO:
		kvm_run_structure_decode_mmio(tcp, vcpu_run_struct);
		break;
...
void
kvm_run_structure_decoder_init(int level)
{
	dump_kvm_run_structure = level;
	mmap_cache_enable();
}
