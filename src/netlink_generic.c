...
static bool genl_decoders_initialized;
static struct {
	const char *family;
...
initialize_genl_decoders(struct tcb *const tcp)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(genl_decoders); i++)
		genl_decoders[i].id = xrlookup(xlat, genl_decoders[i].family, 0);
}
		       ...
		       const unsigned int len)
{
	...
	if (!umove_or_printaddr(tcp, addr, &h)) {
		if (!genl_decoders_initialized) {
			initialize_genl_decoders(tcp);
			genl_decoders_initialized = true;
		}

		netlink_generic_decoder_t decoder = lookup_genl_decoder(nlmsghdr->nlmsg_type);
